...
def evaluate(path: Path, parse_log):
    total = 0
    parsed_success = 0
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Evaluate parser accuracy on a JSONL log dataset."
    )
    parser.add_argument(
        "--jsonl_path",
        type=Path,
    ...
    parser.add_argument("--parser", choices=("llm", "regex"), default="regex")
    ...
    evaluate(args.jsonl_path, parse_log)
